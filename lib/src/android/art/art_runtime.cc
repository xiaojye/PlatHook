...
void ArtRuntime::hookGetOatQuickMethodHeader(void *art_elf_image_) {
     void *symbol = WDynamicLibSymbol(art_elf_image_,
                                             "_ZN3art9ArtMethod23GetOatQuickMethodHeaderEm");
     if (symbol == nullptr)
         symbol = WDynamicLibSymbol(art_elf_image_,
                                           "_ZN3art9ArtMethod23GetOatQuickMethodHeaderEj");
     if (symbol) {
         WInlineHookFunction(symbol, reinterpret_cast<void *>(OnGetOatQuickMethodHeader),
                                    reinterpret_cast<void **>(&GetOatQuickMethodHeaderBackup));
     }
}
